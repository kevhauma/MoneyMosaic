...
export const ReadData = ({onReady}:Props) => {
    const [fieldLegend,setFieldLegend] = useState( ["Saldo","Datum","RekeningNummer"])
    ...
    const convertData = (csvs:string[]) => {
        const csvObjects = csvs.flatMap(csv=>{
            const {legend, list} =csvToArray(csv,{delimiter:';',header:true})
    ...
    const legendOptions = fieldLegend.map(legend=>({value:legend,label:legend}))
    return<Flex vertical>
         <Flex> 
            <Autocomplete placeholder="AmountField" options={legendOptions} value={amountField} onChange={setAmountField}/>
            <Autocomplete placeholder="DateField" options={legendOptions}  value={dateField} onChange={setDateField}/>
            <Autocomplete placeholder="AccountField" options={legendOptions}  value={accountField} onChange={setAccountField}/>
          </Flex>
